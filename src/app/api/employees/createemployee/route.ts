...
export async function POST(req: Request) {
  ...
  try {
    const {
      name,
      role,
      rolePermissions, // Extract role permissions from request
      contact: { email, phone },
    ...
    if (
      ...
      !hoursPerWeek ||
      !shiftPreference ||
      vacationDaysRemaining === undefined
    ) {
    ...
    const newEmployee = await Employee.create({
      ...
      name,
      role,
      rolePermissions, // Save role permissions in the database
      contact: { email, phone },
      availability: {
        hoursPerWeek,
        shiftPreference,
    ...
    const clerkResponse = await fetch("https://api.clerk.com/v1/invitations", {
      ...
      body: JSON.stringify({
        email_address: email,
        public_metadata: { role, employeeId, rolePermissions }, // Add metadata for the invited user
